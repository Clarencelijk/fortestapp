	...
	export default {
		...
		methods: {
			fWindowResize() {
				...
				this.expHeight && (this.exportHeight = this.expHeight.indexOf('rpx') >= 0 ? parseInt(this.expHeight) * this
					.pxRatio :
							...
							success: r => {
								...
								if (!this.hasSel) {
									...
									if (this.selWidth && this.selHeight) {
										let selWidth = this.selWidth.indexOf('rpx') >= 0 ? parseInt(this.selWidth) * this
											.pxRatio :
											parseInt(this.selWidth),
											selHeight = this.selHeight.indexOf('rpx') >= 0 ? parseInt(this.selHeight) * this
											.pxRatio :
